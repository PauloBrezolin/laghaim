<?php
    ...
    $tpl = new TemplatePower( "tpl/tmp_famous.tpl" );
    $tpl->prepare();
    ...
    if($user->login)
    {
        ...
        else
        {
            $query = sprintf("
                ...
                FROM %s.t_users u, %s.t_characters c, %s.t_levelevent_famous f
            ...
            if($dbh->rowCount() >= 1)
            {
                while($r = $dbh->fetch())
                {
                    if(isset($_POST['action']) && $_POST['action'] == 'sendreward' && isset($_POST['cid']) && $_POST['cid'] == $r['a_char_idx'])
                    {
                        ...
                        {
                            if($other[$i]->race == $r['a_char_race'] && $r['a_char_level'] >= $other[$i]->minlevel && $r['a_char_level'] <= $other[$i]->maxlevel)
                                addItem($r['a_user_idx'], $r['a_char_idx'], $other[$i]);
                        }
                        
                        $query = sprintf("UPDATE %s.t_levelevent_famous SET a_taken = 1 WHERE a_index = :idx", Config::DB_WEB);
                        $dbh = $db->prepare($query);
